...
public class NavGrid {
    ...
    public NavGrid(List<Pos> points, List<Obstacle> obstacles, List<Exit> exits, List<HashMap> rooms,
        ...
        for (int i = 0; i < rooms.size(); i++) {
            ...
            for (int k = 0; k < walls1.size(); k++) {
                if (((Number) walls1.get(k).get("x")).doubleValue() > MinPos) {
                    Pos temp_1 = new Pos(((Number) walls1.get(k).get("x")).doubleValue(),
                            ((Number) walls1.get(k).get("y")).doubleValue());
                ...
                if (j != i) {
                    ...
                    for (int k = 0; k < walls.size(); k++) {
                        if (((Number) walls.get(k).get("x")).doubleValue() > MinPos) {
                            Pos temp_1 = new Pos(((Number) walls.get(k).get("x")).doubleValue(),
                                    ((Number) walls.get(k).get("y")).doubleValue());
            ...
            int roomFloorId = rooms.get(i).get("floorId") != null
                    ? ((Number) rooms.get(i).get("floorId")).intValue()
                    : 0;
        ...
        for (int i = 0; i < peosList.size(); i++) {
            ...
            int groupFloorId = peosList.get(i).get("floorId") != null
                    ? ((Number) peosList.get(i).get("floorId")).intValue()
                    : 0;
            temp.setFloorId(groupFloorId);
            points.add(temp);
            populationVertexIndices.add(points.size() - 1);
        }
        matrixRoomToExit = new int[rooms.size() + peosList.size()][exits.size()]; // 先房间后人口框（初始为所有出口，后续为 LP 选取 F1
                                                                                  // 终点时可按需裁剪）
    }

    public NavGrid(int bID, List<Pos> points, List<Obstacle> obstacles, List<Exit> exits, List<HashMap> rooms,
            List<HashMap> peosList,
    ...
    private boolean isIntersectWithRoom(Pos A, Pos B, ArrayList<Integer> room_id) {
        for (int i = 0; i < rooms.size(); i++) {
            ...
            for (int j = 0; j < walls.size() - 1; j++) {
                Pos A1 = new Pos(((Number) walls.get(j).get("x")).doubleValue(),
                        ((Number) walls.get(j).get("y")).doubleValue());
                Pos B1 = new Pos(((Number) walls.get(j + 1).get("x")).doubleValue(),
        ...
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < n; j++) {
                ...
                if (vertex.get(i).room_id.size() > 0 && vertex.get(j).room_id.size() > 0 && vertex.get(i).state == 0
                        ...
                        && !vertex.get(i).room_id.stream().anyMatch(vertex.get(j).room_id::contains)) {
                    matrix[j][i] = MAXLEN;
                }
    ...
    public int anyOneCanReached() {
        ...
        for (int k = 0; k < points.size(); k++) {
            if (remove_rooms.contains(k))
                continue;
            for (int i = 0; i < points.size(); i++) {
                if (remove_rooms.contains(i))
                    continue;
                if (reachable[i]) {
                    continue;
                }
    ...
    public List<Map<String, Integer>> generateLines() throws IOException {
        ...
        // System.out.println("从出口 " + i + " 到房间 " + j + " 的最短路径为: " +
        // pathToAllExit.get(i).get(j));
        // }
        // }
    ...
    public Pos getNavPoint(Pos pos, int exitID) {
        ...
        for (int j = 0; j < rooms.size(); j++) {
            ...
            for (int k = 0; k < walls.size(); k++) {
                if (((Number) walls.get(k).get("x")).doubleValue() > MinPos) {
                    Pos temp = new Pos(((Number) walls.get(k).get("x")).doubleValue(),
                            ((Number) walls.get(k).get("y")).doubleValue());
                    points.add(temp);
                }
            }
        ...
        if (pos.getFloorId() == exit.getCenter().getFloorId()) {
            if (!isIntersectWithObs(pos, exit.getCenter()) && !(isIntersectWithRoom(pos, exit.getCenter(), room_id))
                    && dis > pos.getDistance(exit.getCenter())) {
        ...
        for (int i = 0; i < pathToAllExit.get(exitID).size(); i++) {
            for (int j = 0; j < pathToAllExit.get(exitID).get(i).size(); j++) {
                ...
                if (temp >= vertexID.size())
                    continue;
                Pos targetPos = points.get(vertexID.get(temp));

                // 必须在同一楼层才能直线感知
                if (pos.getFloorId() != targetPos.getFloorId())
                    continue;
        ...
        if (dis < MAXLEN) {
            // System.out.println("目标为导航点" + des.getX() + "出口为" +
            // exits.get(0).getCenter().getX());
            return des;
        }

        for (int i = 0; i < vertexID.size(); i++) {
            Pos targetPos = points.get(vertexID.get(i));
            if (pos.getFloorId() == targetPos.getFloorId() && !(state == 1 && targetPos.state == 0)
                    && !(state == 0 && targetPos.state == 0
                            && !room_id.stream().anyMatch(targetPos.room_id::contains))
                    && !isIntersectWithObs(pos, targetPos)
    ...
    public boolean isToExit(Pos pos) {
        ...
        for (Exit exit : exits) {
            // 必须在同一楼层且无障碍
            if (pos.getFloorId() == exit.getCenter().getFloorId() && !isIntersectWithObs(pos, exit.getCenter())
                    && dis > pos.getDistance(exit.getCenter())) {
                des.setX(exit.getCenter().getX());
                des.setY(exit.getCenter().getY());
                des.setFloorId(exit.getCenter().getFloorId());
        ...
        if (dis < MAXLEN) {
            // System.out.println("点" + pos.x + " " +pos.y +",目标为导航点" + des.getX() + "出口为" +
            // exits.get(0).getCenter().getX());
            return true;
        }
        return false;
