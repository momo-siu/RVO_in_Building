...
public class Simulator {
    ...
    public void setAgentVelocity(int id){
        agents.get(id).velocity = Vector2D.ZERO;
    }

    public void updatePostion(int id, Vector2D pos){
        agents.get(id).position = pos;
    }
...
//            System.out.println("Simulator"+agent.position);
            agent.computeNeighbors();
            agent.computeNewVelocity();
        }
        for (final Agent agent : agents) {
//            System.out.println("Simulator_new:"+agent.position);
        }
        agents.forEach(Agent::update);

        for (final Agent agent : agents) {
//            System.out.println("Simulator_new2:"+agent.position);
        }
        globalTime += timeStep;
    ...
    public int getAgentNumObstacleNeighborsId(int agentNo) {
        return agents.get(agentNo).obstacleNeighbors.size();
    }
    ...
    public void remove(int agentNo) {
        this.agents.remove(agentNo);
    }
...
//    public void setAgentState(int agentNo) {
//        //this.agents.add(agentNo,null);
//        this.agents.set(agentNo,null);
//    }
}
