...
public class KdTree {
    ...
    void buildAgentTree() {
        ...
        if (agents.length != 0) {
//            System.out.println("agents.length:"+agents.length);
    ...
    private static ObstacleTreeNode buildObstacleTreeRecursive(List<Obstacle> obstacles) {
        ...
        for (int j = 0; j < obstacles.size(); j++) {
            ...
            if (j1LeftOfI >= -RVOMath.EPSILON && j2LeftOfI >= -RVOMath.EPSILON) {
                ...
                // Split obstacle j.
                final double t = RVOMath.det(obstacleI2.point.subtract(obstacleI1.point), obstacleJ1.point.subtract(obstacleI1.point)) / RVOMath.det(obstacleI2.point.subtract(obstacleI1.point), obstacleJ1.point.subtract(obstacleJ2.point));
    ...
    /**
     * Recursive method for computing the agent neighbors of the specified
     * agent.
     *
     * @param agent   The agent for which agent neighbors are to be computed.
     * @param rangeSq The squared range around the agent.
    ...
    /**
     * Recursive method for computing the obstacle neighbors of the specified
     * agent.
     *
     * @param agent   The agent for which obstacle neighbors are to be computed.
     * @param rangeSq The squared range around the agent.
    ...
    private static void queryObstacleTreeRecursive(Agent agent, double rangeSq, ObstacleTreeNode node) {
        if (node != null) {
            ...
            final double agentLeftOfLine = RVOMath.leftOf(obstacle1.point, obstacle2.point, agent.position);
